...
describe('SMSDriver', function () {
    ...
    it('throws exception when base URL is not configured', function () {
        $this->app['config']->set('notifyre.base_url', '');
    ...
    it('throws exception when API key is not configured', function () {
        $this->app['config']->set('services.notifyre.api_key', '');
        $this->app['config']->set('notifyre.api_key', '');
    ...
    it('prioritizes services.notifyre.api_key over notifyre.api_key', function () {
        $this->app['config']->set('notifyre.api_key', 'fallback-key');
        $this->app['config']->set('services.notifyre.api_key', 'primary-key');
    ...
    it('uses configured timeout and retry settings', function () {
        $this->app['config']->set('notifyre.timeout', 60);
        $this->app['config']->set('notifyre.retry.times', 5);
        $this->app['config']->set('notifyre.retry.sleep', 2000);
        ...
        expect($this->app['config']->get('notifyre.timeout'))->toBe(60)
            ->and($this->app['config']->get('notifyre.retry.times'))->toBe(5)
            ->and($this->app['config']->get('notifyre.retry.sleep'))->toBe(2000);
    });

    it('trims trailing slash from base URL', function () {
        $this->app['config']->set('notifyre.base_url', 'https://api.notifyre.com/');
